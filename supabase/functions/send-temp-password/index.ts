...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Send temp password via NextSMS
    ...
    const smsResponse = await fetch('https://messaging-service.co.tz/api/sms/v1/text/single', {
      ...
      body: JSON.stringify({
        from: senderId,
        to: formattedPhone,
        text: `Your Vitabu temporary password is: ${tempPassword}. Use it to login and change your password immediately.`,
      }),
    });
    ...
    console.log(`Temporary password sent successfully to ${phone}`);

    return new Response(
      JSON.stringify({ success: true, message: 'Temporary password sent successfully' }),
